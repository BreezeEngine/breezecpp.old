...
namespace ecs {
namespace core {
  //static_assert(has_duplicates<Components...>(), "component_group contains duplicates");
...
struct component_group {
  ...
  template <class... Ts>
  auto get_range_with_index(){
    auto r = get_range<Ts...>();
    constexpr size_t zero = 0;
    return ranges::view::zip(r,ranges::view::closed_ints(0ul,ranges::size(r)));
  }
  template <class... Ts>
  auto get_range()
  {
    auto filter = hana::tuple_t<Container<Ts>...>;
    const auto components_ref = hana::transform(
        components_tuple, [&](auto&& c) { return std::ref(c); });
    auto components_filter = hana::filter(components_ref, [&](auto&& cref) {
      return contains_type(filter, cref.get());
    });
    auto scf = hana::sort(components_filter, [&](auto&& a, auto&& b) {
    ...
    auto components_filter_range = hana::transform(
        scf, [](auto&& cref) { return ranges::view::all(cref.get()); });
...
struct component_groups {
  //static_assert(has_duplicates<ComponentGroups...>(), "component_groups contains duplicates");
};

template <class World, class... Systems>
struct systems_group {
  World& world;
  ...
  void update()
  {
    hana::for_each(systems_tuple, [&](auto& s) { s.update(world); });
  }
};
template <class World, class... Systems>
...
struct world<component_groups<ComponentGroups...>> {
  ...
  auto get_range()
  {
    auto filter_tup = hana::tuple_t<std::vector<Ts>...>;
    auto cg_ref = tuple_ref(component_groups_tuple);
    auto cg_ref_filtered = hana::filter(cg_ref, [&](auto&& cg) {
      return contains_all_types(filter_tup, cg.get().components_tuple);
    });
